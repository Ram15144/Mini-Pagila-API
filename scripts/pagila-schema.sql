...
-- CREATE DATABASE pagila WITH TEMPLATE = template0 ENCODING = 'UTF8' LC_COLLATE = 'en_US.UTF-8' LC_CTYPE = 'en_US.UTF-8';
...
CREATE OR REPLACE FUNCTION film_in_stock(p_film_id integer, p_store_id integer, OUT p_film_count integer) RETURNS SETOF integer
    LANGUAGE sql
    AS $_$
     SELECT inventory_id
     FROM inventory
...
CREATE OR REPLACE FUNCTION film_not_in_stock(p_film_id integer, p_store_id integer, OUT p_film_count integer) RETURNS SETOF integer
    LANGUAGE sql
    AS $_$
    SELECT inventory_id
    FROM inventory
...
CREATE OR REPLACE FUNCTION last_day(timestamp without time zone) RETURNS date
    LANGUAGE sql IMMUTABLE STRICT
    AS $_$
  SELECT CASE
    WHEN EXTRACT(MONTH FROM $1) = 12 THEN
      (((EXTRACT(YEAR FROM $1) + 1)::text || '-01-01')::date - INTERVAL '1 day')::date
    ELSE
      ((EXTRACT(YEAR FROM $1)::text || '-' || (EXTRACT(MONTH FROM $1) + 1)::text || '-01')::date - INTERVAL '1 day')::date
    END
$_$;
...
-- streaming_subscription table will be added in Migration #3
...
ALTER TABLE ONLY rental ALTER COLUMN rental_id SET DEFAULT nextval('rental_rental_id_seq'::regclass);
...
-- streaming_subscription defaults will be added in Migration #3
...
-- streaming_subscription constraints will be added in Migration #3
...
CREATE INDEX idx_fk_film_id ON film_category USING btree (film_id);
...
-- streaming_subscription triggers will be added in Migration #3
...
-- streaming_subscription foreign keys will be added in Migration #3
